...
st.write("Skin Disease Detection with Deep Learning")
...
def transform_img_fn(img, path_list):
    ...
    for img_path in path_list:
        img_ = image.load_img(img_path, target_size=(150, 150))
...
if file is None:
    ...
    if np.argmax(prediction) == 0:
        st.write("Prediction: Actinic Keratosis")
    elif np.argmax(prediction) == 1:
        st.write("Prediction: Cherry Angioma")
    elif np.argmax(prediction) == 2:
        st.write("Prediction: Dermatofibroma")
    elif np.argmax(prediction) == 3:
        st.write("Prediction: Melanoma")
    elif np.argmax(prediction) == 4:
        st.write("Prediction: Pityriasis Rosea")

    elif np.argmax(prediction) == 5:
        st.write("Prediction: Rash Form Plants")
    elif np.argmax(prediction) == 6:
        st.write("Prediction: Seborrheic Keratosis")
    elif np.argmax(prediction) == 7:
        st.write("Prediction: Squamous Cell Carcinoma")  
    elif np.argmax(prediction) == 8:
        st.write("Prediction: VaricoseVeins")
    elif np.argmax(prediction) == 9:
        st.write("Prediction: Warts")  
    else:
        st.write("Prediction: Invaild")
    
    st.text("Probability (0: Actinic Keratosis, 1: Cherry Angioma, 2: Dermatofibroma, 3: Melanoma, 4: Pityriasis Rosea, 5: Rash Form Plants, 6: Seborrheic Keratosis, 7: Squamous Cell Carcinoma,8: VaricoseVeins, 9: Warts")
